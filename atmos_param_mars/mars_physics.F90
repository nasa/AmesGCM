...
!
!
!  Module that contains main physics driver and initialization for Mars GCM
!
...
use mars_surface_mod,       only: mars_surface_init, mars_surface_end, &
                                 ...
                                 id_frost, sfc_roughness, sfc_frost_micro, &
                                 id_frost_micro, sfc_h2o2_chem, id_sfc_h2o2_chem
...
use aerosol_util_mod,       only: do_moment_micro, init_aerosol_flags, do_moment_sedim

#ifdef RELEASE
use null_physics_mod, only: init_null_phys
use null_physics_mod, only: micro_driver, micro_driver_init, dust_update_init,   &
          dust_update, dust_update_end, sfc_dust_mass, &
          dust_update_init, dust_update, dust_update_end, &
...
use null_physics_mod, only: palmer_drag, palmer_drag_init, palmer_drag_end, &
...
logical :: no_forcing = .false.                 ! skip mars_physics
logical :: do_mars_surface = .true.             ! do mars surface calculations

real :: t_zero=315., t_strat=200., delh=60., delv=10., eps=0., sigma_b=0.7

real :: ka = -4.                               ! Newton Damping timescale negative values are damping time in days
real :: ks =  -4.                              ! Special case of Newton timescale negative values are damping time in days
real :: kf = -4000.e6                          ! Rayleigh Damping timescale negative values are damping time in days

logical :: do_conserve_energy = .false.         ! adjust temperature according to wind drag
real :: trflux = 1.e-5                          ! surface flux for optional tracer
real :: trsink = -4.                            ! damping time for tracer

logical :: sponge_flag = .false.                 ! flag for Rayleigh damping at the top of model
real    :: sponge_tau_days  = 1.0               ! damping time scale for the sponge (days)
real    :: rflevel = 0.1                        ! Raleigh damping pressure level (Pa)
real    :: sponge_pbottom = 1.0                 ! bottom of sponge layer,below which Rayleight damping is zero (Pa)

logical :: sponge_flag2 = .false.                 ! flag for Rayleigh damping at the top of model
real    :: sponge_tau_days2  = 1.0               ! damping time scale for the sponge (days)
real    :: rflevel2 = 0.1                        ! Raleigh damping pressure level (Pa)
real    :: sponge_pbottom2= 1.0                 ! bottom of sponge layer,below which Rayleight damping is zero (Pa)
real    :: rfwidth2= 30.0                        ! bottom of sponge layer,below which Rayleight damping is zero (Pa)


logical  :: do_vert_diff = .true.               ! do vertical diffusion
logical  :: do_ames_pbl = .true.                ! do ames MY2.0 PBL

logical  :: do_mars_radiation = .true.          ! do mars radiation
logical  :: do_simple_radiation = .false.       ! do simple mars radiation
logical  :: do_teq_z = .false.                  ! do newton damping forcing
logical  :: do_convective_adjust = .true.       ! do convective adjustment
logical  :: do_fv3_convect = .true.             ! do original full column adjustment

logical  :: do_co2_condensation_cycle = .true.  ! do mass feedback from condensation
logical  :: do_co2_condensation = .true.        ! do atmospheric co2 condensation
logical  :: do_atmos_co2_loss = .true.          ! do atmospheric co2 loss from condensation
...
logical  :: diffusion_smooth = .true.           ! smooth diffusion over tau_diffusion time scale
...
logical  :: inj_vap_pbl = .true.                ! inject water vapor throughout PBL
real     :: facsubl = 1.0                       ! water vapor sublimation factor

logical  :: do_pchem = .false.                   ! switch for photochemistry
logical  :: do_qmass = .false.                  ! switch for using multiple gases in total atm mass
!
...
integer  :: id_teq, id_tdt, id_tdt2, id_udt, id_vdt,id_tdt_diss, id_diss_heat, id_tdt_hrad,id_tdt_pbl,id_tdt_adj,id_tdt_micro
...
integer  :: id_tsfc, id_stress, id_stress_gw, id_udt_cgwd, id_vdt_cgwd,id_tdt_cgwd, &
            id_zpbl, id_difft,id_ppbl,id_kpbl,id_sens,id_wflux_vap, id_wflux_vap_bin, id_precip
integer  :: id_udt_topo, id_vdt_topo
integer  :: id_uv,  id_vt, id_uw, id_dnflux
integer  :: id_rdt_h2o2, id_cond_mass,id_rdt_subl_bin
integer, dimension(:),  allocatable :: id_rdt_pbl,id_rdt_adj,id_rdt_dst,id_rdt_micro,id_rdt_dif,&
                                       id_rdt_hrad,id_rdt_subl,id_rdt_sedim,id_rdt_coag
...
real, dimension(size(r,1),size(r,2),size(r,3),size(r,4))   :: rdt_rad,rdt_pbl,rdt_adj &
               ,rdt_dst,rdt_micro,rdt_dif,rdt_cld,rdt0,rdt_subl,rdt_subl_bin,rdt_sedim,rdt_dss,rdt_coag
...
real, dimension(size(t,1),size(t,2)) :: frost_micro, &                     ! moment micro surface water ice [kg/m2]
                                        frost_eff                          ! bin or moment depending on compiler flag [kg/m2]
real, dimension(size(t,1),size(t,2), nice_mass) :: frost_micro0, &         ! initial water ice field for conservation [kg/m2]
                                                   frost_field             ! temporary water ice field for updates [kg/m2]
real, dimension(size(t,1),size(t,2), ndust_mass) :: sfc_dst_micro0         ! initial dust field for conservation [kg/m2]
...
nh2o_mom= find_field_index( MODEL_ATMOS, 'vap_mass_micro' )
nh2o_bin= find_field_index( MODEL_ATMOS, 'h2o_vapor' )
nice_mom= find_field_index( MODEL_ATMOS, 'ice_mass_micro' )

if (do_mars_surface) then
if (do_moment_micro) then
    nh2o2  = find_field_index( MODEL_ATMOS, 'h2o2_mmr_gas' )
    frost_micro0=sfc_frost_micro(is:ie,js:je,:)
    sfc_dst_micro0=sfc_dust_mass(is:ie,js:je,:)
    frost_eff(:,:)=sfc_frost_micro(is:ie,js:je,1)
...
if (checkcons) then
    ...
    call checkconserv(is,ie,js,je,kd,p_half,r,frost_micro0,sfc_dst_micro0,p_half,r+rdt*dt,sfc_frost_micro,sfc_dust_mass,'initest')
endif
...
!************************************************
    frost_field(:,:,:)=sfc_frost_micro(is:ie,js:je,:)
    call update_water(is,ie,js,je,kd,lat,lon,dt,p_half,tsurf,rhouch_save,k_pbl_save, &
                    r(:,:,:,:),rdt(:,:,:,:),nh2o_mom,frost_field,wflux_vap,rdt_subl,sols)
    sfc_frost_micro(is:ie,js:je,:)=frost_field

    if (id_wflux_vap > 0) used = send_data ( id_wflux_vap, wflux_vap, Time, is, js )

    frost_field(:,:,:)=sfc_frost(is:ie,js:je,:)
    call update_water_fv3(is,ie,js,je,kd,lat,lon,dt,p_half,tsurf,rhouch_save,k_pbl_save, &
                    r(:,:,:,:),rdt(:,:,:,:),nh2o_bin,frost_field(:,:,1),wflux_vap,rdt_subl_bin)
    ...
    if (checkcons) then
        ...
        call checkconserv(is,ie,js,je,kd,p_half,r,frost_micro0,sfc_dst_micro0,p_half, &
                    r+rdt*dt,sfc_frost_micro,sfc_dust_mass,'updatew')
    endif
...
! For now, zero out moisture fluxes   !    These would be used by the soil model also
    evap(:,:)= 0.0
    devap_datmos(:,:)= 0.0
...
!-----------------------------------
        if (do_ames_pbl) then
            ...
            if (checkcons) then
                ...
                call checkconserv(is,ie,js,je,kd,p_half,r,frost_micro0,sfc_dst_micro0,p_half, &
                                r+rdt*dt,sfc_frost_micro,sfc_dust_mass(:,:,:),'amespbl')
            endif
...
! Progts assumes downward flux has a positive sign
    ...
    if (checkcons) then
        call checkconserv(is,ie,js,je,kd,p_half,r,frost_micro0,sfc_dst_micro0,p_half, &
                        r+rdt*dt,sfc_frost_micro,sfc_dust_mass(:,:,:),'preprog')
    endif
...
!    This is the temperature seen by the radiation code
    ...
    if (checkcons) then
        call checkconserv(is,ie,js,je,kd,p_half,r,frost_micro0,sfc_dst_micro0,p_half, &
                        r+rdt*dt,sfc_frost_micro,sfc_dust_mass(:,:,:),'progtsf')
    endif
...
!***************** IF DO_SURFACE_MARS = FALSE
     ...
     if (do_vert_diff) then
        ...
        if (checkcons) then
            call checkconserv(is,ie,js,je,kd,p_half,r,frost_micro0,sfc_dst_micro0,p_half, &
                            r+rdt*dt,sfc_frost_micro,sfc_dust_mass(:,:,:),'diffuse')
        endif
    endif
...
!-----------------------------------------------------------------------
 ...
 if (do_pchem)   then !photochemistry is on
    ...
    if (checkcons) then
      call ckconchem(is,ie,js,je,kd,p_half,r,frost_micro0,sfc_h2o2_chem0,p_half,r+rdt*dt,sfc_frost_micro,sfc_h2o2_chem,'pkem')
    endif
  ...
  !************************************************
    ...
    !! test conservation tracers
    if (checkcons) then
      call ckconchem(is,ie,js,je,kd,p_half,r,frost_micro0,sfc_h2o2_chem0,p_half,r+rdt*dt,sfc_frost_micro,sfc_h2o2_chem,'H2O2')
    endif
...
if (checkcons) then
    call checkconserv(is,ie,js,je,kd,p_half,r,frost_micro0,sfc_dst_micro0,p_half, &
                            r+rdt*dt,sfc_frost_micro,sfc_dust_mass(:,:,:),'pretopo')
endif
...
if (nml_switch(GW_drag_TOG,3)) then !Alexander 1999
    ...
    call cg_drag_calc (is, js, lat, p_full, z_full, t+tdt*dt, u+udt*dt, &
                      v+vdt*dt, Time, dt, udt_top, &
                      vdt_top, tdt_top )
    udt =  udt + udt_top
    vdt =  vdt + vdt_top
    tdt=   tdt + tdt_top
...
if (checkcons) then
    call checkconserv(is,ie,js,je,kd,p_half,r,frost_micro0,sfc_dst_micro0,p_half, &
                    r+rdt*dt,sfc_frost_micro,sfc_dust_mass(:,:,:),'prevadj')
endif
...
if (checkcons) then
    ...
    call checkconserv(is,ie,js,je,kd,p_half,r,frost_micro0,sfc_dst_micro0,p_half, &
                    r+rdt*dt,sfc_frost_micro,sfc_dust_mass(:,:,:),'convadj')
endif
...
if (do_moment_micro) then
    call dust_update ( is, js, lon, lat, dt, Time,     &
    						...
    						sfc_frost_micro(is:ie,js:je,1),stress, taudust_mom, t, tdt, r, rdt, &
    						sens, rdt_dst, source_mom, lifting_dust )
    rdt=rdt+rdt_dst
    ...
    if (checkcons) then
        ...
        call checkconserv(is,ie,js,je,kd,p_half,r,frost_micro0,sfc_dst_micro0,p_half, &
                        r+rdt*dt,sfc_frost_micro,sfc_dust_mass(:,:,:),'dustupd')
    endif
...
if (do_moment_micro) then
!    if (do_moment_sedim) call sedim_driver(is,js,Time,p_half,p_full,t,tdt,r,rdt,tsurf,dt,rkh,rdt_sedim,checkcons)
!    rdt=rdt+rdt_sedim
    ...
    rdt=rdt+rdt_micro
    tdt=tdt+tdt_micro
!test moving sedim after micro
    if (do_moment_sedim) call sedim_driver(is,js,Time,p_half,p_full,t,tdt,r,rdt,tsurf,dt,rkh,rdt_sedim,lifting_dust,checkcons)
    rdt=rdt+rdt_sedim

    do nt= 1, nice_mass
        if (id_frost_micro(nt) > 0)  used = send_data ( id_frost_micro(nt), sfc_frost_micro(is:ie,js:je,nt), time, is, js )
    enddo
...
if (checkcons) then
    ...
    call checkconserv(is,ie,js,je,kd,p_half,r,frost_micro0,sfc_dst_micro0,p_half, &
                    r+rdt*dt,sfc_frost_micro,sfc_dust_mass(:,:,:),'microph')
endif
...
do n= 1, ntrace
    if( id_rdt_pbl(n) > 0 )  used = send_data( id_rdt_pbl(n),  &
                                 rdt_pbl(is:ie,js:je,:,n), time, is, js )
    if( id_rdt_adj(n) > 0 )  used = send_data( id_rdt_adj(n),  &
                                 rdt_adj(is:ie,js:je,:,n), time, is, js )
    if( id_rdt_dst(n) > 0 )  used = send_data( id_rdt_dst(n),  &
                                 rdt_dst(is:ie,js:je,:,n), time, is, js )
    if( id_rdt_micro(n) > 0 )  used = send_data( id_rdt_micro(n),  &
                                 rdt_micro(is:ie,js:je,:,n), time, is, js )
    if( id_rdt_sedim(n) > 0 )  used = send_data( id_rdt_sedim(n),  &
                                 rdt_sedim(is:ie,js:je,:,n), time, is, js )
    if( id_rdt_coag(n) > 0 )  used = send_data( id_rdt_coag(n),  &
                                 rdt_coag(is:ie,js:je,:,n), Time, is, js )
    if (id_rdt_hrad(n) > 0) used = send_data ( id_rdt_hrad(n), rdt_rad(is:ie,js:je,:,n), time, is, js )
enddo
do nt= 1, nice_mass
    ndx=vapor_indx(nt)
...
do nt= 1, nice_mass
    ...
    id_rdt_subl(nt) = register_diag_field ( mod_name, trim(tname), axes(1:3), Time, &
              'Water micro Sublimation tendency', 'kg/kg/s'   , &
                missing_value=missing_value )
enddo
...
id_difft = register_diag_field ( mod_name, 'difft', axes(1:3), Time, &
              'diffusion temperature tendency', 'k/s',  &
               missing_value=missing_value     )

id_difft_ames = register_diag_field(mod_name, 'difft_ames', axes(half),Time, &
             'eddy heat mixing coefficient', 'K/s' ,&
             missing_value=missing_value)

id_tdt_hrad = register_diag_field ( mod_name, 'tdt_hrad', axes(1:3), Time, &
              'Radiative heating rate', 'degrees K'   , &
                missing_value=missing_value )

id_tdt_pbl = register_diag_field ( mod_name, 'tdt_pbl', axes(1:3), Time, &
              'tendency Temp PBL', 'degrees K/s'  , &
                missing_value=missing_value )
id_tdt_adj = register_diag_field ( mod_name, 'tdt_adj', axes(1:3), Time, &
              'tendency Temp conv adjustment', 'degrees K/s'  , &
                missing_value=missing_value )

id_tdt_micro = register_diag_field ( mod_name, 'tdt_micro', axes(1:3), Time, &
              'tendency Temp microphysics', 'degrees K/s'  , &
                missing_value=missing_value )

id_lheat = register_diag_field ( mod_name, 'tdt_co2', axes(1:3), Time, &
              ' CO2 latent heating ', 'deg_K/sec',  &
               missing_value=missing_value     )


!!! DIFFUSION
id_diffm_ames = register_diag_field(mod_name, 'diffm_ames', axes(half), Time, &
             'eddy momentum mixing coef', 'm2/s' ,&
             missing_value=missing_value)
...
id_vmass = register_diag_field ( mod_name, 'vmass', axes(1:3), Time, &
              'meridional mass flux', 'kg/s/s/s'   , &
                missing_value=missing_value )

id_uv = register_diag_field ( mod_name, 'uv', axes(1:3), Time, &
              'uv momentum flux', 'm2/s2'   , &
                missing_value=missing_value )

id_vt = register_diag_field ( mod_name, 'vt', axes(1:3), Time, &
              'vt heat flux', 'Km/s'   , &
                missing_value=missing_value )

id_uw = register_diag_field ( mod_name, 'uw', axes(1:3), Time, &
              'vertical momentum flux', 'kg/s/s/s'   , &
                missing_value=missing_value )

!!! PBL
id_zpbl = register_diag_field ( mod_name, 'zpbl', axes(1:2), Time, &
              ' Boundary layer depth', 'm',  &
               missing_value=missing_value     )

id_kpbl = register_diag_field ( mod_name, 'kpbl', axes(1:2), Time, &
              'Level of Boundary layer depth', 'lev', &
               missing_value=missing_value     )

id_ppbl = register_diag_field ( mod_name, 'ppbl', axes(1:2), Time, &
              ' Boundary layer depth', 'mbar',  &
               missing_value=missing_value     )
...
id_tsfc = register_diag_field ( mod_name, 'tsfc', axes(1:2), Time, &
              'Surface temperature', 'degrees K'   , &
                missing_value=missing_value )

id_dnflux = register_diag_field ( mod_name, 'dnflux', axes(1:2), Time, &
              'Total Downward rad flux', 'W m-2'   , &
                missing_value=missing_value )
...
id_stress = register_diag_field ( mod_name, 'stress', axes(1:2), Time, &
              'Surface stress', 'N/m/m'   , &
                missing_value=missing_value )

id_precip = register_diag_field ( mod_name, 'precip', axes(1:2), Time, &
              'Atmospheric CO2 snow', 'kg/m2'   , &
                missing_value=missing_value )

!!! WATER CYCLE
id_wflux_vap = register_diag_field ( mod_name, 'wflux_vap', axes(1:2), Time, &
              'Water micro Sublimation flux', 'kg/m2/s'   , &
                missing_value=missing_value )

id_wflux_vap_bin = register_diag_field ( mod_name, 'wflux_vap_bin', axes(1:2), Time, &
              'Water bin Sublimation flux', 'kg/m2/s'   , &
                missing_value=missing_value )
...
id_rdt_h2o2 = register_diag_field ( mod_name, 'h2o2_Tcond', axes(1:3), Time, &
                  'Tendency H2O2 Condensation', 'kg/kg/s' ,    &
                   missing_value=missing_value     )

id_cond_mass = register_diag_field ( mod_name, 'cond_mass', axes(1:3), Time, &
          'Condensed H2O2 Mass', 'kg/m2' ,    &
           missing_value=missing_value     )
...
if (nml_switch(GW_drag_TOG,1)) then
    ...
    id_stress_gw = register_diag_field ( mod_name, 'base_flux', axes(1:2), Time, &
              'Surface stress', 'N/m/m'   , &
                missing_value=missing_value )
endif
...
if (nml_switch(GW_drag_TOG,3)) then
    ...
    id_udt_cgwd =  register_diag_field ( mod_name, 'udt_cgwd', axes(1:3),  &
                      Time, 'utend for cg wave drag',   'm/s2',   &
                       missing_value=missing_value               )
    id_vdt_cgwd =  register_diag_field ( mod_name, 'vdt_cgwd', axes(1:3),  &
                      Time, 'vtend for cg wave drag',   'm/s2',   &
                       missing_value=missing_value               )
    id_tdt_cgwd =  register_diag_field ( mod_name, 'tdt_cgwd', axes(1:3),  &
                      Time, 'ttend for cg wave drag',   'm/s2',   &
                       missing_value=missing_value               )
...
if( do_moment_micro) then
    ...
    call micro_driver_init(nlon, mlat, nlevels, lonb, latb, lon, lat, axes, Time )
...
if( do_moment_micro ) call dust_update_end
...
qarray(:,:,:)=  0.0
qdt(:,:,:)=   0.0
!          The following are not actually used; so they are dummy arrays
frac_land(:,:)= 1.0
tdtlw(:,:,:)= 0.0
convect(:,:)= .false.
q_star(:,:)= 0.0
...
rini(:,:,:,:)=r(:,:,:,:)+rdt(:,:,:,:)*dt
rdt_subl(:,:,:,:)=0.
qgnd(:,:)  = (18.0/44.0)*611.0*exp(22.5*(1.0-(273.16/tg(:,:))))/pl(:,:,nz+1)
...
do nt=1,nice_mass
    ...
    call tagging_main( (/ "geosource" , "loctime", "antigeosource" , "cutsource" /) &
