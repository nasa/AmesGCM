...
!           impose soil ice ti
    ...
    case ( 1  )
        ...
        where ( abs(lat) >  55.0 * pi/180.0   )
            gk3d(:,:,5 )= ( 4.0*gk(:,:) + 1.0*gk_ice )/5.0
            gk3d(:,:,6 )= ( 3.0*gk(:,:) + 2.0*gk_ice )/5.0
            gk3d(:,:,7 )= ( 2.0*gk(:,:) + 3.0*gk_ice )/5.0
            gk3d(:,:,8 )= ( 1.0*gk(:,:) + 4.0*gk_ice )/5.0
            gk3d(:,:,9 )= gk_ice
            gk3d(:,:,10)= gk_ice
            gk3d(:,:,11)= gk_ice
            gk3d(:,:,12)= gk_ice
    ...
    case ( 2  )
        ...
        where ( abs(lat) >  65.0 * pi/180.0   )
            gk3d(:,:,3)= ( 4.0*gk(:,:) + 1.0*gk_ice )/5.0
            gk3d(:,:,4)= ( 3.0*gk(:,:) + 2.0*gk_ice )/5.0
            gk3d(:,:,5)= ( 2.0*gk(:,:) + 3.0*gk_ice )/5.0
            gk3d(:,:,6 )= gk_ice
            gk3d(:,:,7)= gk_ice
            gk3d(:,:,8)= gk_ice
        end where
    ...
    case ( 3  )
        ...
        if( nlay > 12 ) then
            gk3d(:,:,13:nlay)= gk_ice2
        endif

        where ( lat >  55.0 * pi/180.0   )
            gk3d(:,:,5 )= ( 4.0*gk(:,:) + 1.0*gk_ice )/5.0
            gk3d(:,:,6 )= ( 3.0*gk(:,:) + 2.0*gk_ice )/5.0
            gk3d(:,:,7 )= ( 2.0*gk(:,:) + 3.0*gk_ice )/5.0
            gk3d(:,:,8 )= ( 1.0*gk(:,:) + 4.0*gk_ice )/5.0
        ...
        where ( lat <  -55.0 * pi/180.0   )
            gk3d(:,:,8 )= ( 4.0*gk(:,:) + 1.0*gk_ice )/5.0
            gk3d(:,:,9 )= ( 3.0*gk(:,:) + 2.0*gk_ice )/5.0
            gk3d(:,:,10)= ( 2.0*gk(:,:) + 3.0*gk_ice )/5.0
            gk3d(:,:,11)= gk_ice
            gk3d(:,:,12)= gk_ice2
        end where
...
#ifdef TI_SOIL_ICE3
        tice_thresh= 200.0
        if( abs(rlat) > 55.0 ) then
            do kk= 3, 7
                gk3d(:,kk)= gk(:)
                do i= 1, i2
                    if( tg(i,kk) <  tice_thresh )  gk3d(i,kk)= gk_ice
                enddo
            enddo
        endif
#endif TI_SOIL_ICE3
    ...
    case ( 6 )
        ...
        do i= 1, id
            do j= 1, jd
                if(  lat(i,j) >  60.0 * pi/180.0  ) tice_thresh= 175.0
                if(  lat(i,j) >  70.0 * pi/180.0  ) tice_thresh= 185.0
                if(  lat(i,j) >  80.0 * pi/180.0  ) tice_thresh= 200.0


                do kk= 3, 7
                    gk3d(i,j,kk)= gk(i,j)
                    if( tg(i,j,kk) <  tice_thresh )  gk3d(i,j,kk)= gk_ice
                enddo
